<template>
  <div class="event" :class="{ 'is-current': isCurrentEvent }">
    <div class="hour-info">
      ...
      >
        <p v-if="isCurrentEvent" v-bind:class="{ 'is-current': isCurrentEvent }">
    ...
    <div
      class="item radient-border-passthrough"
      :class="'type-' + event.type.replace(/\s/g, '').replace(/\W/g, '').toLowerCase()"
    ...
    >
      ...
      <div class="div-foto">
        <div v-if="speakers_image_list.length > 0">
          ...
          <div
            v-if="companies_image_list.length > 0"
            id="small-foto"
            class="radient-border-passthrough"
          >
        ...
        <div v-else>
          ...
          >
            <img
              style="border-radius: 0%; height: 70%; width: 70%"
              src="@/assets/jeec_mobile_white.svg"
              alt="JEEC"
            />
          </div>
        </div>
  ...
  methods: {
    ...
    getCompaniesImageList(event) {
      ...
      for (let i = 0; i < event.companies.data.length; i++) {
        image_list.push(import.meta.env.VITE_APP_JEEC_BRAIN_URL + event.companies.data[i].logo)
      }
      return image_list
    },
    ...
    getSpeakersImageList(event) {
      ...
      for (let i = 0; i < event.speakers.data.length; i++) {
        image_list.push(import.meta.env.VITE_APP_JEEC_BRAIN_URL + event.speakers.data[i].image)
      }
      return image_list
    },
...
.item {
  ...
  --background: var(
    ...
    radial-gradient(
      ...
      rgba(76, 202, 240, 0.14) 0%,
      rgba(76, 202, 240, 0.08) 70%,
      rgba(76, 202, 240, 0) 100%
    )
  );
  --border-background: var(
    --border-background_,
    linear-gradient(165deg, #605ed0 0%, #4cc9f0 40%, #7209b7 100%)
  );
...
.item.type-insidetalks {
  --color: #f72585;
  --background_: radial-gradient(
    ellipse 250% 200% at 0% 0%,
    rgba(247, 37, 132, 0.14),
    rgba(247, 37, 132, 0.08) 60%,
    rgba(247, 37, 132, 0)
  );
...
.item.type-cerimony {
  ...
  --background_: radial-gradient(
    ellipse 250% 200% at 0% 0%,
    rgba(26, 156, 216, 0.14),
    rgba(26, 156, 216, 0.08) 60%,
    rgba(26, 156, 216, 0)
  );
...
.item.type-fastmeeting {
  ...
  --background_: radial-gradient(
    ellipse 250% 200% at 0% 0%,
    rgba(221, 64, 221, 0.14),
    rgba(221, 64, 221, 0.08) 60%,
    rgba(221, 64, 221, 0)
  );
  --border-background_: linear-gradient(165deg, #a414a4, #f72585 40%, #7209b7);
}
